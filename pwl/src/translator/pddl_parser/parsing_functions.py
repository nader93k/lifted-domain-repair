...
def parse_action(alist, type_dict, predicate_dict):
    #TODO: this is a hack for the move domain -- should be more general
    pot_pos = [i for i, s in enumerate(alist) if s == ':precondition']
    assert len(pot_pos) < 2
    if len(pot_pos) == 1:
        pre_pos = pot_pos[0]
        if alist[pre_pos+1][0] == ':':
            alist.insert(pre_pos+1, [])
