...
def eff_to_atom(eff):
    name = ('delpred_' if eff.literal.negated else 'addpred_') + eff.literal.predicate
    variables = [par for par in eff.literal.args]
...
class PreconditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules):
        ...
        if options.integrate_add_del_to_pre:
            for eff in action.effects:
                rule_body.append(eff_to_atom(eff))
        if options.integrate_action_cost:
            rule_head.predicate.name += '__with__cost__' + str(action.cost if action.cost is not None else 1)
